...
- Для `JSON` также читает `budgets`, `distribution_items`, `distribution_subitems` и `distribution_snapshots`.
...
- Для `JSON` full backup может восстанавливать budgets, структуру распределения и frozen distribution snapshots через `replace_budgets(...)`, `replace_distribution_structure(...)` и `replace_distribution_snapshots(...)`.

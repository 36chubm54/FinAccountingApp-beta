...
- `JSON` full backup теперь может включать `distribution_items`, `distribution_subitems` и `distribution_snapshots`; при импорте они восстанавливаются в SQLite, если репозиторий поддерживает Distribution System.
...
Этот экспорт теперь сохраняет `mandatory_expenses.date`, структуру Distribution System и frozen `distribution_snapshots`.
...
- `db/schema.sql` — SQL-схема таблиц `wallets`, `records`, `transfers`, `mandatory_expenses`, `budgets`, `distribution_*`.
...
- `domain/` — бизнес‑модели и правила (записи, бюджеты, отчёты, аудит данных, валидация дат и периодов, валюты, кошельки, transfers).
...
- `services/` — сервисный слой для импорта, read-only аудита, бюджетов и аналитики SQLite.
...
- SQL-схема БД: таблицы `wallets`, `records`, `transfers`, `mandatory_expenses`, `budgets`, ограничения и индексы.
...
- Для `JSON` также читает `distribution_items`, `distribution_subitems` и `distribution_snapshots`.
...
- Для `JSON` full backup может восстанавливать и структуру распределения, и frozen distribution snapshots через `replace_distribution_structure(...)` / `replace_distribution_snapshots(...)`.

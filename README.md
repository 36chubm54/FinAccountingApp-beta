...
Текущий релиз `v1.9.1` закрепляет debt-aware backup/import flow: добавляет совместимость со старой SQLite-схемой до `v1.9.0`, формализует `ImportedBackupData` для low-level backup parsing и сохраняет `import_backup(...)` только как deprecated compatibility wrapper.
...
# Опционально: установка PDF-экспорта (reportlab + pillow)
pip install -r requirements-pdf.txt
# или, если установка идёт как пакета:
# pip install .[pdf]
...
- `JSON` full backup теперь может включать `budgets`, `debts`, `debt_payments`, `distribution_items`, `distribution_subitems` и `distribution_snapshots`; при импорте они восстанавливаются в SQLite, если репозиторий поддерживает соответствующие подсистемы.
...
- Для pre-`1.9.0` SQLite БД bootstrap сначала добавляет `records.related_debt_id`, и только потом применяет полную схему с debt-indexes.
...
- Для `JSON` при `ImportPolicy.CURRENT_RATE` bulk replace path теперь тоже разрешён, если репозиторий поддерживает fast replace, чтобы не терять debt/debt-payment связи.
...
- `ImportedBackupData` — структурированный результат low-level backup parsing (`wallets`, `records`, `mandatory_expenses`, `transfers`, дополнительные payloads и `summary`).
- `import_full_backup_from_json(filepath, force=False)` — основной low-level helper для backup JSON parsing; читает также `debts` / `debt_payments` и возвращает `ImportedBackupData`.
- `import_backup(filepath, force=False)` — deprecated compatibility wrapper над `import_full_backup_from_json(...)`.
...
├── requirements.txt                  # Базовые runtime-зависимости
├── requirements-pdf.txt              # Опциональные PDF-зависимости (`reportlab` + transitive `pillow`)

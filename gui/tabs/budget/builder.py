...
from .contracts import BudgetTabBindings, BudgetTabContext
from .list_section import _draw_progress_bars
    ...
    *,
    ...
    def _display_forecast_status(result: BudgetResult) -> str | None:
        ...
        if "amount_base" in params:
            params["amount_base"] = format_display_money(
                float(params["amount_base"]),
                precision=0,
            )
        fallbacks = {
    ...
    def _refresh() -> None:
        ...
        try:
            if scope_type_var.get() == "tag":
                category_combo["values"] = [
                    str(getattr(tag, "name", "")) for tag in context.controller.list_tags()
                ]
            else:
        ...
        budget_tree.heading(
            "remaining",
            text=f"{tr('budget.remaining', 'Остаток')} ({display_currency()})",
        )

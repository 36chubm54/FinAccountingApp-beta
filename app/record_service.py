...
class RecordService:
    ...
    def update_record_inline(
        ...
        *,
        ...
        updated_amount_base = to_money_float(float(new_amount_base))
        if str(record.currency or self._base_currency_code()).upper() == self._base_currency_code():
            updated = replace(
                record,
                amount_original=updated_amount_base,
                amount_base=updated_amount_base,
                rate_at_operation=1.0,
            )
        else:
            updated = record.with_updated_amount_base(updated_amount_base)
    ...
    def update_mandatory_amount_base(self, expense_id: int, new_amount_base: float) -> None:
        try:
            new_amount_base = float(new_amount_base)
        ...
        updated = expense.with_updated_amount_base(new_amount_base)

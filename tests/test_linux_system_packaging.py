...
def test_stage_system_package_rootfs_copies_bundle_and_assets(tmp_path: Path) -> None:
    ...
    metainfo_entry = rootfs / "usr" / "share" / "metainfo" / "finaccountingapp.metainfo.xml"
    ...
    assert '<launchable type="desktop-id">finaccountingapp.desktop</launchable>' in metainfo_content
...
def test_render_metainfo_xml_includes_readme_summary_and_release_notes() -> None:
    ...
    assert '<launchable type="desktop-id">finaccountingapp.desktop</launchable>' in content
...
def test_write_rendered_nfpm_config_replaces_placeholders(tmp_path: Path) -> None:
    ...
    assert "/usr/share/metainfo/finaccountingapp.metainfo.xml" in content
